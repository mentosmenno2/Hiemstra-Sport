<?php
	if (strlen($postedEmail) <= 45)
	{
		if (strlen($postedPassword) >= 4 && strlen($postedPassword) <= 20)
		{
									...
									if (strlen($postedTown) <= 45)
									{
										if (strlen($postedPhone) <= 45)
										{
											if (strlen($postedSports) <= 65535)
											{
												...
												{
													require 'functions/checkregisterform/checkRegistrationFormValuesAreValid.php';
												}
												else
												{
		...
		{
			displayMessage("Uw wachtwoord moet tussen de 4 en 20 tekens lang zijn.");
		}
	}
	else
	{
		displayMessage("Uw emailadres mag maar 45 tekens lang zijn.");
	}
?> 